...
Capacities are finite, non-negative `float` values. `AddNode()` rejects a terminal-capacity aggregate that exceeds the `float` range. `AddEdge()` and `ComputeMaxFlow()` reject an edge whose paired residual capacities do not have a finite sum, since augmentation transfers capacity between the two directions; `ComputeMaxFlow()` also rejects a node whose incident paired capacities do not have a finite sum, which bounds its batched source-side deficit. A validation exception leaves the construction graph unchanged, so invalid direct slot capacities can be corrected before retrying.
