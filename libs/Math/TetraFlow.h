...
public:
	...
	// throws if either aggregate would exceed the finite range of Cap
	void AddNode(NodeID n, Cap capSource, Cap capSink) {
		assert(n < nodes.size());
		Node& node = nodes[n];
		ValidateCapacitySum(node.excess, capSource);
		ValidateCapacitySum(node.capSink, capSink);
	...
	void AddEdge(NodeID u, NodeID v, Cap capUV, Cap capVU) {
		...
		ValidateResidualPair(capUV, capVU);
		Node& nu = nodes[u];
		Node& nv = nodes[v];
	...
	// the time ComputeMaxFlow() is called, and each linked pair must have a finite sum
...
private:
	...
	static void ValidateCapacity(Cap c) {
		if (c > std::numeric_limits<Cap>::max())
			throw std::overflow_error("TetraFlow capacity exceeds float range");
		if (!(c >= 0))
			throw std::invalid_argument("TetraFlow capacity must be finite and non-negative");
	}
	static void ValidateCapacitySum(Cap first, Cap second) {
		ValidateCapacity(first);
		ValidateCapacity(second);
		const Cap large = std::max(first, second);
		const Cap small = std::min(first, second);
		if (small > std::numeric_limits<Cap>::max() - large)
			throw std::overflow_error("TetraFlow combined capacities exceed float range");
	}
	static void ValidateResidualPair(Cap capForward, Cap capReverse) { ValidateCapacitySum(capForward, capReverse); }
	...
	void ValidateGraph() const {
		for (size_t x = 0; x < nodes.size(); ++x) {
			...
			ValidateCapacity(node.excess);
			ValidateCapacity(node.capSink);
			...
			for (unsigned i = 0; i < NUM_SLOTS; ++i) {
				...
				const NodeID y = node.head[i];
				assert(y < nodes.size() && y != x);
				const unsigned j = Rev(node, i);
				assert((nodes[y].fill & Bit(j)) && nodes[y].head[j] == x && Rev(nodes[y], j) == i);
				const Cap reverseCap = nodes[y].rcap[j];
				ValidateCapacity(cap);
				ValidateCapacity(reverseCap);
				if (x < y)
					ValidateResidualPair(cap, reverseCap);
				incidentCapacity += double(cap) + double(reverseCap);
				if (incidentCapacity > double(std::numeric_limits<Cap>::max()))
					throw std::overflow_error("TetraFlow node capacity exceeds float range");
			}
		}
	}
