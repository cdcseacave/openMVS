...
**RoMa v2 (optional, `CreateStructure --roma2`)**: an in-process ONNX Runtime deployment of RoMa v2 (a DINOv3 descriptor graph + a bidirectional coarse-match graph, no refiner) offers two independent passes over the classical matching above. Pair-selection ranking is a property of `--match-mode` alone, with no crossover between the two backends:

| `--match-mode` | ranking backend | model needed |
|---|---|---|
| `1` VOCABULARY | the SIFT/AKAZE vocabulary tree, **always** | no |
| `4` RETRIEVAL | the DINOv3+GeM(p=3) 2048-D global descriptors, **always** | only to compute them |

RETRIEVAL ranks candidate pairs using the descriptor graph's own on-device pooling of its output as the global descriptor per image (`RoMa2Onnx::Describe`'s `retrieval` output, read by `GlobalDescriptors.h`); `--roma2` only needs to compute those descriptors when the scene does not already carry them, and RETRIEVAL runs with `--roma2-match false` (the default) — robust DINOv3 pair selection feeding the ordinary SIFT/AKAZE/ORB matching, no dense matching required. `--roma2-match` (**default off**) replaces the descriptor matching round outright with one dense pass per candidate pair (A, B): the coarse-match graph returns both warps from a single call; the pair is admitted only if the smaller of the two inlier areas one fitted geometry explains — measured on A's confident cells and, through the B→A warp, on B's — reaches `--roma2-min-overlap` (0.10, about 0.15–0.17 of true overlap); the described keypoints of A are then tracked into B through the warp and matched inside a disc of two warp cells, each winner having to beat the best keypoint OUTSIDE the disc by the matcher's ratio; the rest of the confident overlap is filled at `--roma2-dense-matches` (2000) correspondences per FULL FRAME of overlap — a density, not a per-pair count — drawn from the part of the verdict's confident overlap the guided matches did not already cover, and capped by that same density over the smaller of the two inlier areas so the draw stays bounded in both images; and one geometry is fitted on the union. A pair the verdict rejects is dropped — there is no fallback to descriptor matching for it, and no union of SIFT-verified and warp-verified pairs. Needs an exported model (`--roma2-model DIR`, default `$OPENMVS_ROMA2_MODEL_PATH`, see `docs/RoMa2Model.md` for how to obtain it) and a build with `-DOpenMVS_USE_ONNXRUNTIME=ON`. Flags: `--roma2`, `--roma2-model`, `--roma2-setting turbo|fast|base`, `--roma2-match`, `--roma2-slots N`, `--roma2-min-confidence F`, `--roma2-min-overlap F`, `--roma2-dense-matches N`, `--roma2-provider auto|cuda|coreml|dml|cpu`, `--export-retrieval-csv` (like `--export-pairs-csv`, written right after matching, before reconstruction; needs `--roma2 true`). See `docs/design/ROMA2InProcess.md` for the full design.

**Triplet view-graph disambiguation (on by default, `CreateStructure --filter-triplets`, `ViewGraphTriplets.h`)**: a reimplementation of Manam & Govindu (CVPR 2024) that removes wrong pairs — repeated structure, retrieval false positives — from the matched graph using nothing but the graph and one strength per edge, its epipolar inlier count discounted by the inliers' grid coverage. The triangles of the view graph form the *triplet graph* (two triangles adjacent iff they share an edge); the edges taking part in its largest connected component are scored by the mean, over their triangles, of `s_ij / max_{(k,l) in t} s_kl`, and a triangle whose three pairs all yield below 0.4 of the graph's own envelope gives no evidence; unscored pairs — including every pair in no triangle at all — carry no evidence either way and are kept. `tau(m)` is a ceiling, not the threshold itself: the default `m` is 0.3, the paper's value for the medium and small ambiguous sets; a stricter ceiling at the second-face score 0.75 is tried first and, when the graph it leaves is two-faced (a majority piece with a second piece of at least a third of it), names the faces: the paper's ceiling then applies inside the larger face and every pair joining the other face is cut, and below the ceiling the threshold is the strictest one joining every piece unless the largest piece already holds a majority; the reconstruction seeds in the largest ceiling piece. It runs right after matching (and right after `--export-pairs-csv`, so the CSV lists every matched pair with its score in a `TripletScore` column), then recomputes the pair weights. It is complementary to the rotation-cycle-consistency weight `ImagePair::weightTriplet`, which by construction cannot catch doppelgangers — their false edges are mutually consistent — and both are kept. **Two modes.** By default the ceiling only names the pieces: the candidates are the scored pairs below it that join two pieces the ceiling keeps apart, and of those only what the graph can spare goes -- every image keeps at least 3 pairs holding 2,000 weighted inliers at 3 degrees or more of ray angle, the threshold descends to the strictest one at most half the images fall short of (a graph fitting none loses nothing), and every component of the graph stays whole -- so a scene the ceiling leaves in one piece (every orbit measured) or that fits no threshold (every interior measured) loses nothing, and a collection the ceiling splits loses the joins it can spare. `--triplet-cut` is the paper's rule with the face rule and the descent, which unfolds a symmetric building and halves an interior's registrations. Flags: `--filter-triplets B`, `--triplet-cut B`, `--triplet-keep-pairs N`, `--triplet-keep-matches N`, `--triplet-keep-min-angle F`, `--triplet-keep-max-short F`, `--triplet-auto-tau B`, `--triplet-min-score F`, `--triplet-second-face-score F`. See `docs/design/TripletDisambiguation.md` for the measurements.
...
`Scene::Load` accepts **that exact version only**, so a `.sfm` written by any other layout is refused
with `error: unsupported SFM project version N (this build reads only version M) in '<file>'`. This
branch's stream layout is **version 1**. There is no converter and none is planned: a file written by
another layout is regenerated by re-running the matching stage from the images
(`CreateStructure -s <images> -o scene.sfm ...`), never converted. `.mvs` files are unaffected.
...
├── ViewGraphTriplets.h/cpp             # Camera-triplet view-graph disambiguation (on by default; --triplet-cut unfolds)
│
│ # Reconstruction
