...
**Triplet view-graph disambiguation (optional, `CreateStructure --filter-triplets`, `ViewGraphTriplets.h`)**: a reimplementation of Manam & Govindu (CVPR 2024) that removes wrong pairs — repeated structure, retrieval false positives — from the matched graph using nothing but the graph and one integer per edge, its epipolar inlier count. The triangles of the view graph form the *triplet graph* (two triangles adjacent iff they share an edge); the edges taking part in its largest connected component are scored by the mean, over their triangles, of `n_ij / max_{(k,l) in t} n_kl`, and removed only if that score falls below `tau = m*(1 - d_max/|V|) + d_max/|V|` with `|V|` and `d_max` taken on that scored sub-graph; a pair outside that scored sub-graph — including every pair in no triangle at all — carries no evidence either way and is kept. It runs right after matching (and right after `--export-pairs-csv`, so the CSV lists every matched pair with its score in a `TripletScore` column), then recomputes the pair weights. It is complementary to the rotation-cycle-consistency weight `ImagePair::weightTriplet`, which by construction cannot catch doppelgangers — their false edges are mutually consistent — and both are kept. **Default off**: on the five validation captures the score separates true from false pairs well (ROC AUC 0.61-0.89 against depth-derived ground truth) but the paper's threshold is calibrated for internet photo collections, not for video keyframes, and at `m = 0.6` it discards 56-76 % of the verified pairs and costs 5-76 % of the registered images (and `m = 0.3` still 2-78 %). Flags: `--filter-triplets B`, `--triplet-min-score F` (the paper's *m*, in (0,1); lower removes fewer edges). See `docs/design/TripletDisambiguation.md` for the measurements.
