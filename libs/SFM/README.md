...
**Triplet view-graph disambiguation** (on by default, `CreateStructure --filter-triplets`, `ViewGraphTriplets.h`): reimplements Manam & Govindu (CVPR 2024): each pair is scored by its coverage-discounted inlier count relative to the strongest edge of every triangle it belongs to, and the threshold is `tau(m)=m(1-r)+r` (default `m` 0.3). **Two modes**: by default only pairs joining pieces the threshold keeps apart go, and only what the graph can spare; `--triplet-cut` unfolds a symmetric building at an interior's expense. Complementary to `ImagePair::weightTriplet`. Flags: `--filter-triplets B`, `--triplet-cut B`, `--triplet-min-score F`, `--triplet-keep-pairs N`, `--triplet-keep-matches N`, `--triplet-keep-min-angle F`, `--triplet-keep-max-short F`, `--triplet-second-face-score F`. See `docs/design/TripletDisambiguation.md`.
...
├── ViewGraphTriplets.h/cpp             # Camera-triplet view-graph disambiguation (keep mode by default, --triplet-cut cuts)
│
│ # Reconstruction
