...
#ifdef _MSC_VER
  static const char* const separator_set = "\\/";
...
// folder functions
  ...
  std::vector<std::string> folder_subdirectories (const std::string& directory, bool recursive)
  {
    return folder_wildcard(directory, "*", recursive, true, false);
  }

  std::vector<std::string> folder_files (const std::string& directory, bool recursive)
  {
    return folder_wildcard(directory, "*", recursive, false, true);
  }

  std::vector<std::string> folder_all(const std::string& directory, bool recursive)
  {
    return folder_wildcard(directory, "*", recursive, true, true);
  }

  std::vector<std::string> folder_wildcard (const std::string& directory, const std::string& wild, bool recursive, bool subdirs, bool files)
  {
    std::string dir = directory.empty() ? std::string(".") : directory;
    ...
    for (bool OK = (handle = _findfirst((char*)wildcard.c_str(), &fileinfo)) != -1; OK; OK = (_findnext(handle, &fileinfo)==0))
    {
      ...
      if (strentry.compare(".")!=0 && strentry.compare("..")!=0) {
        if (fileinfo.attrib & _A_SUBDIR) {
          if (recursive) {
            const std::string relpath(strentry+preferred_separator);
            const std::vector<std::string> sub_results(folder_wildcard(directory+relpath, wild, recursive, subdirs, files));
            for (const std::string& file: sub_results)
    ...
    if (d)
    {
      for (dirent* entry = readdir(d); entry; entry = readdir(d))
      {
        ...
        if (strentry.compare(".")!=0 && strentry.compare("..")!=0)
        {
          ...
          if (is_folder(subpath) && wildcard(wild, strentry)) {
            if (recursive) {
              const std::string relpath(subpath+preferred_separator);
              const std::vector<std::string> sub_results(folder_wildcard(directory+relpath, wild, recursive, subdirs, files));
              for (const std::string& file: sub_results)
                results.push_back(relpath+file);
            }
          ...
          if (files && is_file(subpath) && wildcard(wild, strentry))
            results.push_back(strentry);
        }
      }
      closedir(d);
    ...
    if (recursive && wild.compare("*")!=0) {
      const std::vector<std::string> sub_folder(folder_subdirectories(directory, true));
      for (const std::string& folder: sub_folder) {
        const std::string relpath(folder+preferred_separator);
        const std::vector<std::string> sub_results(folder_wildcard(directory+relpath, wild, true, subdirs, files));
        for (const std::string& file: sub_results)
          results.push_back(relpath+file);
      }
   }
