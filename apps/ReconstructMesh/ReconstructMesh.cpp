...
bool Application::Initialize(size_t argc, LPCTSTR* argv)
{
	...
	config_main.add_options()
		...
		("constant-weight", boost::program_options::value(&OPT::bUseConstantWeight)->default_value(false), "consider all view weights 1 instead of the per-view point confidence the point-cloud carries (a point-cloud without confidence always votes 1)")
