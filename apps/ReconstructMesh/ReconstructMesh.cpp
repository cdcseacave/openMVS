...
bool Application::Initialize(size_t argc, LPCTSTR* argv)
{
	...
	config_clean.add_options()
		...
		("max-edge-scale", boost::program_options::value(&OPT::fMaxEdgeScale)->default_value(4.f), "remove faces whose longest edge exceeds this factor times the local edge scale, the median edge length around the face's vertices (its 3-ring), before any other cleaning (0 - disabled)")
