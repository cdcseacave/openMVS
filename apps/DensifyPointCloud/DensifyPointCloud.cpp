...
bool Application::Initialize(size_t argc, LPCTSTR* argv)
{
	...
	config.add_options()
		...
		("postprocess-dmaps", boost::program_options::value(&nOptimize)->default_value(8), "flags used to filter the depth-maps after estimation (0 - disabled, 1 - remove-speckles, 2 - fill-gaps, 4 - adjust-confidence, 8 - adjust-confidence only when the depth-maps are estimated on CUDA, where it runs fused into the last estimation iteration and costs almost nothing; the default 8 therefore enables it on GPU and skips it on CPU, where it would cost a separate full-resolution pass -- pass 4 to force it on regardless)")
