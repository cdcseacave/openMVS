...
bool Application::Initialize(size_t argc, LPCTSTR* argv)
{
	...
	config.add_options()
		...
		("fast", boost::program_options::bool_switch(&OPT::bFast)->default_value(false), "trade accuracy for speed: refine against 4 neighbor images and decimate the result within 0.5 pixels (1.5-1.7x faster for -0.006 mean F1 and a 1.8-2.7x smaller mesh); an explicitly given --max-views or --simplify-tolerance wins over it")
