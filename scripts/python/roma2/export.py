...
def write_manifest(args):
    ...
    """
    import onnx
    from graphs import FACET_BLOCKS, LAYER_IDX, PATCH
...
def main():
    ...
    pm.add_argument("--exporter", default="dynamo", choices=["dynamo", "torchscript"],
                    help="recorded as-is: which exporter actually produced these graphs")
    pm.add_argument("--value-facet-blocks", type=block_list,
                    help="recorded as-is: the blocks the descriptor graph was traced with (default 15,20)")
    pm.add_argument("--checkpoint", default=DEFAULT_CHECKPOINT, help="hashed into checkpoint_sha256")
    pm.add_argument("--roma2-repo", default=DEFAULT_ROMA2_REPO,
                    help="the checkout the graphs were traced from: its HEAD and the DINOv3 hub revision "
                         "it pins go into the manifest")
