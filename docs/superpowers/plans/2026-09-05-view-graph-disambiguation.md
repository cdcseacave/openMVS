...
### Task 28: The design note and the README describe the filter as it ships: on by default, two modes, the campaign

**Files:**
- Modify: `docs/design/TripletDisambiguation.md` (the overview's last sentence; a new section `## Two modes` between `## The algorithm` and `## Where it runs, and the flags`; the flags table and the Python line in `## Where it runs, and the flags`; the whole of `## The default, and why`; the first and the fourth bullet of `## Limitations and follow-ups`)
- Modify: `libs/SFM/README.md` (the paragraph beginning `**Triplet view-graph disambiguation` and the tree line for `ViewGraphTriplets.h/cpp`)

**Interfaces:**
- Consumes: the flags and defaults of `apps/CreateStructure/CreateStructure.cpp` (`--filter-triplets` true, `--triplet-cut` false, `--triplet-keep-pairs` 3, `--triplet-keep-matches` 2000, `--triplet-keep-min-angle` 3, `--triplet-keep-max-short` 0.5, `--triplet-auto-tau` true, `--triplet-min-score` 0.3, `--triplet-second-face-score` 0.75); the Python binding's field order in `libs/SFM/PythonWrapper.cpp` (`enabled, auto_tau, cut, keep_pairs, keep_matches, keep_min_angle, keep_max_short, min_score, second_face_score, min_yield`); the log lines of `libs/SFM/ViewGraphTriplets.cpp`.
- Produces: nothing code-facing.

This task changes prose only; there is no test to write. The numbers below are the campaign's (spec §5.10); copy them as given.

- [ ] **Step 1: The overview's last sentence**

In `docs/design/TripletDisambiguation.md`, replace
...
- [ ] **Step 2: The new section `## Two modes`**

Insert the following section immediately before the line `## Where it runs, and the flags` (leave one blank line on each side):
...
- [ ] **Step 3: The flags table and the Python line**

In `## Where it runs, and the flags`, replace the table from its header line `| Flag | Default | Effect |` through the row beginning `` | `--triplet-second-face-score F` `` with
...
(If the Python line in the file differs from the old text above in anything but the field list, keep its other words and change only the field list.)

- [ ] **Step 4: `## The default, and why`**

Replace the whole section, from the line `## The default, and why` up to (not including) the line `## Limitations and follow-ups`, with:
...
- [ ] **Step 5: The limitations**

In `## Limitations and follow-ups`, replace the first bullet (from `* **Auto-enabling is the obvious next step.**` through `that manual flag too.`) with these three bullets:
...
and replace the bullet beginning `* **One model per run**:` with
...
In `libs/SFM/README.md`, in the paragraph beginning `**Triplet view-graph disambiguation`, replace

```
(optional, `CreateStructure --filter-triplets`, `ViewGraphTriplets.h`)
```

with

```
(on by default, `CreateStructure --filter-triplets`, `ViewGraphTriplets.h`)
```

and replace the text from `**Default off**:` up to (not including) `See `docs/design/TripletDisambiguation.md`` with

```
**Two modes.** By default the ceiling only names the pieces: the candidates are the scored pairs below it that join two pieces the ceiling keeps apart, and of those only what the graph can spare goes -- every image keeps at least 3 pairs holding 2,000 weighted inliers at 3 degrees or more of ray angle, the threshold descends to the strictest one at most half the images fall short of (a graph fitting none loses nothing), and every component of the graph stays whole -- so a scene the ceiling leaves in one piece (every orbit and interior measured) loses nothing, and a collection the ceiling splits loses the joins it can spare. `--triplet-cut` is the paper's rule with the face rule and the descent, which unfolds a symmetric building and halves an interior's registrations. Flags: `--filter-triplets B`, `--triplet-cut B`, `--triplet-keep-pairs N`, `--triplet-keep-matches N`, `--triplet-keep-min-angle F`, `--triplet-keep-max-short F`, `--triplet-auto-tau B`, `--triplet-min-score F`, `--triplet-second-face-score F`. 
```

(keep the sentence `See `docs/design/TripletDisambiguation.md` for the measurements.` that follows). Replace the tree line
...
Run `/usr/bin/grep -n -E 'off by default|opt-in filter|Default off|Auto-enabling' docs/design/TripletDisambiguation.md libs/SFM/README.md` and expect no output. Then:

```bash
git add docs/design/TripletDisambiguation.md libs/SFM/README.md
git -c user.name=cDc -c user.email=cdc.seacave@gmail.com commit -m "docs: the triplet filter ships on in the keep mode; the design note and the README say what each mode removes and what the campaign measured"
```
