...
- **Configuration:** `ROMA2Config` (`--roma2*` CLI flags on `CreateStructure`) — `enabled`, `modelPath`/`$OPENMVS_ROMA2_MODEL_PATH`, `setting`, `provider`, `useRetrieval`, `useMatching`, `minConfidence`, `minOverlap`, `denseMatchesPerFrame`, `slotBudget`
