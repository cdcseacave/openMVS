...
A second option for estimating the dense point-cloud is a hierarchical Semi-Global Matching algorithm (tSGM) on the CPU, combining *Memory Efficient Semi-Global Matching* H. Hirschmüller et al. 2012 for the cost aggregation with *SURE: Photogrammetric surface reconstruction from imagery* M. Rothermel et al. 2012 for the coarse-to-fine scheme: each image pair is stereo-rectified and matched with a weighted zero-mean NCC cost aggregated along 8 paths, the pair disparity-maps are fused into a depth-map per image, and the standard depth-map fusion produces the point-cloud. On the EPFL ground-truth scenes its precision is close to the Patch-Match approach's while its recall is lower (F-score 0.33/0.26 vs 0.40/0.27 on Herz-Jesu-P8/fountain-P11), and on the CPU it is faster than Patch-Match. The algorithm as implemented, the measurements behind the defaults and the planned improvements are recorded in [design/SemiGlobalMatching.md](https://github.com/cdcseacave/openMVS/blob/master/docs/design/SemiGlobalMatching.md).
