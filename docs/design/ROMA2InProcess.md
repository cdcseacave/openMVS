...
`GlobalDescriptors::PoolRetrievalDescriptor` (`libs/SFM/GlobalDescriptors.h/cpp`) pools one image's
...
- **Self-calibration from warp-guided matches is weaker than from descriptor matches.** A guided match
  ...
  on `--use-global-solver`/uncalibrated refinement alone; measured on real captures in Task 11.
