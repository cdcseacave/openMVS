...
- **Integration point:** Alternative to `PairsMatcher::MatchFeatures()`. **Addressed for RoMa v2:** the in-process RoMa v2 dense matcher (`--roma2*` on `CreateStructure`; `libs/SFM/MatchROMA2.h/cpp`, `libs/SFM/ROMA2Warp.h/cpp`; see `docs/design/ROMA2InProcess.md`) runs its coarse dense warps through ONNX Runtime and turns each into a guided sparse re-match of the candidate pair, replacing the classical descriptor match only when it has strictly more inliers -- it supplements, never replaces, SIFT/AKAZE/ORB. LightGlue/LoFTR/MASt3R as alternative detector-free matchers remain open.
