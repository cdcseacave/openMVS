...
- **Integration point:** New `FeatureType` enum entries in `FeaturesExtractor.h`. **Partially addressed:** the ONNX Runtime inference wrapper this needs now exists and runs in production as part of the in-process RoMa v2 integration (`libs/SFM/OnnxRuntime.h/cpp`, `libs/SFM/RoMa2Matcher.h/cpp`; see `docs/design/ROMA2InProcess.md`). Adding a learned single-image feature extractor (SuperPoint/ALIKED/DISK/DeDoDe) as a new `FeatureType` is still open, but the wrapper is no longer new engineering risk.
- **Complexity:** Medium
- **Priority:** High
...
- **Integration point:** Alternative to `PairsMatcher::MatchFeatures()`. **Addressed for RoMa v2:** the in-process RoMa v2 dense matcher (`--roma2-match` on `CreateStructure`; `libs/SFM/MatchROMA2.h/cpp`, `libs/SFM/ROMA2Warp.h/cpp`, `libs/SFM/MatchGeometric.h/cpp`; see `docs/design/ROMA2InProcess.md`) runs one bidirectional coarse-match call per candidate pair through ONNX Runtime, admits or rejects the pair by the min-side inlier area of one fitted geometry, and on admission guides a sparse match under the warp, fills the remaining overlap densely, and fits one union geometry -- a rejected pair is dropped rather than left to SIFT/AKAZE/ORB. LightGlue/LoFTR/MASt3R as alternative detector-free matchers remain open.
